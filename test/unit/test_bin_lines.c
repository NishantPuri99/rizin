...
#undef SAMPLES_COUNT
			...
			case 0:
				...
				break;
			case 1:
				rz_bin_source_line_info_builder_push_sample(&bob, 0x1001, 42, 5, "mayan.c");
				break;
			case 2:
			...
			case 7:
				...
				break;
			case 8:
				rz_bin_source_line_info_builder_push_sample(&bob, 0x1010, 10, 100, "panoramas.c");
				break;
			case 9:
...
#define SAMPLES_COUNT 0x200
		...
		for (size_t i = 0; i < SAMPLES_COUNT; i++) {
			...
			if (rand() % 10 > 2) {
				...
				samples[i].file = test_filenames[rand() % RZ_ARRAY_SIZE(test_filenames)];
			}
		...
		for (size_t i = 0; i < SAMPLES_COUNT; i++) {
			...
			if (!s->line) {
				for (size_t j = 0; j < SAMPLES_COUNT; j++) {
					...
					if (samples[j].address == s->address && samples[j].line) {
						// ...unless the sample is a closing one and overwritten by a
						// non-closing one at the same address.
						goto skip;
					}
				}
			}
		...
		for (size_t i = 0; i < li->samples_count; i++) {
			...
			for (size_t j = 0; j < SAMPLES_COUNT; j++) {
				...
				if (a->address == s->address &&
					((!a->line && !s->line) || (a->line == s->line && a->column == s->column && ((!a->file && !s->file) || (a->file && s->file && !strcmp(a->file, s->file)))))) {
					found = true;
					break;
				}
			}
