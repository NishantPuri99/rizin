...
static void store_line_sample(RzBinSourceLineInfoBuilder *bob, const RzBinDwarfLineHeader *hdr, RzBinDwarfSMRegisters *regs,
	...
	ut32 line = (ut32)regs->line;
	if (!line) {
		// line == 0 can occur in dwarf, but for us it's reserved for closing entries
		// just use line 1 instead.
		line = 1;
	}
	rz_bin_source_line_info_builder_push_sample(bob, regs->address, line, (ut32)regs->column, file);
}
